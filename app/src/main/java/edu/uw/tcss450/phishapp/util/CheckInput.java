package edu.uw.tcss450.phishapp.util;
...
public class CheckInput {
    ...
    private void checkPassword(String theString) {
        if(!checkForEmptyPassword(theString)) {
            theErrorMessage = theErrorMessage.concat("Invalid Password: password is empty\n");
            theState = false;
        }
        if(!checkForShortPassword(theString)) {
            theErrorMessage = theErrorMessage.concat("Invalid Password: password is to short");
            theState = false;
        }
    }
    ...
    private void checkEmail(String theString) {
        if(!checkForAtEmail(theString)) {
            theErrorMessage = theErrorMessage.concat("Invalid Email: contains no @\n");
            theState = false;
        }
        if(!checkForEmptyEmail(theString)) {
            theErrorMessage = theErrorMessage.concat("Invalid Email: email is empty");
            theState = false;
        }
