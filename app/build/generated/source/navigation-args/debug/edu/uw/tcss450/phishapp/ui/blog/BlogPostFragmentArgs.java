package edu.uw.tcss450.phishapp.ui.blog;

import android.os.Bundle;
import android.os.Parcelable;
import androidx.annotation.NonNull;
import androidx.navigation.NavArgs;
import java.io.Serializable;
import java.lang.IllegalArgumentException;
import java.lang.Object;
import java.lang.Override;
import java.lang.String;
import java.lang.SuppressWarnings;
import java.util.HashMap;

public class BlogPostFragmentArgs implements NavArgs {
  private final HashMap arguments = new HashMap();

  private BlogPostFragmentArgs() {
  }

  private BlogPostFragmentArgs(HashMap argumentsMap) {
    this.arguments.putAll(argumentsMap);
  }

  @NonNull
  @SuppressWarnings("unchecked")
  public static BlogPostFragmentArgs fromBundle(@NonNull Bundle bundle) {
    BlogPostFragmentArgs __result = new BlogPostFragmentArgs();
    bundle.setClassLoader(BlogPostFragmentArgs.class.getClassLoader());
    if (bundle.containsKey("blog")) {
      BlogPost blog;
      if (Parcelable.class.isAssignableFrom(BlogPost.class) || Serializable.class.isAssignableFrom(BlogPost.class)) {
        blog = (BlogPost) bundle.get("blog");
      } else {
        throw new UnsupportedOperationException(BlogPost.class.getName() + " must implement Parcelable or Serializable or must be an Enum.");
      }
      if (blog == null) {
        throw new IllegalArgumentException("Argument \"blog\" is marked as non-null but was passed a null value.");
      }
      __result.arguments.put("blog", blog);
    } else {
      throw new IllegalArgumentException("Required argument \"blog\" is missing and does not have an android:defaultValue");
    }
    return __result;
  }

  @SuppressWarnings("unchecked")
  @NonNull
  public BlogPost getBlog() {
    return (BlogPost) arguments.get("blog");
  }

  @SuppressWarnings("unchecked")
  @NonNull
  public Bundle toBundle() {
    Bundle __result = new Bundle();
    if (arguments.containsKey("blog")) {
      BlogPost blog = (BlogPost) arguments.get("blog");
      if (Parcelable.class.isAssignableFrom(BlogPost.class) || blog == null) {
        __result.putParcelable("blog", Parcelable.class.cast(blog));
      } else if (Serializable.class.isAssignableFrom(BlogPost.class)) {
        __result.putSerializable("blog", Serializable.class.cast(blog));
      } else {
        throw new UnsupportedOperationException(BlogPost.class.getName() + " must implement Parcelable or Serializable or must be an Enum.");
      }
    }
    return __result;
  }

  @Override
  public boolean equals(Object object) {
    if (this == object) {
        return true;
    }
    if (object == null || getClass() != object.getClass()) {
        return false;
    }
    BlogPostFragmentArgs that = (BlogPostFragmentArgs) object;
    if (arguments.containsKey("blog") != that.arguments.containsKey("blog")) {
      return false;
    }
    if (getBlog() != null ? !getBlog().equals(that.getBlog()) : that.getBlog() != null) {
      return false;
    }
    return true;
  }

  @Override
  public int hashCode() {
    int result = 1;
    result = 31 * result + (getBlog() != null ? getBlog().hashCode() : 0);
    return result;
  }

  @Override
  public String toString() {
    return "BlogPostFragmentArgs{"
        + "blog=" + getBlog()
        + "}";
  }

  public static class Builder {
    private final HashMap arguments = new HashMap();

    public Builder(BlogPostFragmentArgs original) {
      this.arguments.putAll(original.arguments);
    }

    public Builder(@NonNull BlogPost blog) {
      if (blog == null) {
        throw new IllegalArgumentException("Argument \"blog\" is marked as non-null but was passed a null value.");
      }
      this.arguments.put("blog", blog);
    }

    @NonNull
    public BlogPostFragmentArgs build() {
      BlogPostFragmentArgs result = new BlogPostFragmentArgs(arguments);
      return result;
    }

    @NonNull
    public Builder setBlog(@NonNull BlogPost blog) {
      if (blog == null) {
        throw new IllegalArgumentException("Argument \"blog\" is marked as non-null but was passed a null value.");
      }
      this.arguments.put("blog", blog);
      return this;
    }

    @SuppressWarnings("unchecked")
    @NonNull
    public BlogPost getBlog() {
      return (BlogPost) arguments.get("blog");
    }
  }
}
